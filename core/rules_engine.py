...
DEFAULT_SETTLEMENT_RULES: list[SettlementRule] = [
    SettlementRule("VIL",  5000, 0.45, 0.40, 0.50, 0.45, review_column=32, mark_am=True, value_to_review="PRELIT", enabled=True, description="VIL — threshold $5,000"),
    SettlementRule("CKS",  5000, 0.45, 0.40, 0.50, 0.45, review_column=32, mark_am=True, value_to_review="PRELIT", enabled=True, description="CKS — threshold $5,000"),
    SettlementRule("FFAM", 5000, 0.45, 0.40, 0.50, 0.45, z_greater_than_threshold=True, enabled=True, description="FFAM — only applies if Z > 0.45"),
    SettlementRule("OLU",  5000, 0.45, 0.40, 0.50, 0.45, enabled=True, description="OLU — threshold $5,000"),
    SettlementRule("OLI",  5000, 0.45, 0.40, 0.50, 0.45, enabled=True, description="OLI — threshold $5,000"),
    SettlementRule("CAG",  5000, 0.45, 0.40, 0.50, 0.45, enabled=True, description="CAG — threshold $5,000"),
    SettlementRule("LPL",  0,    0.45, 0.45, 0.45, 0.45, mark_aq=True, enabled=True, description="LPL — fixed rate, mark AQ"),
    SettlementRule("GAM",  1250, 0.45, 0.40, 0.50, 0.45, z_lower_than_threshold=True, enabled=True, description="GAM — threshold $1,250"),
    SettlementRule("SMF",  0,    0.50, 0.45, 0.50, 0.45, z_lower_than_threshold=True, enabled=True, description="SMF — fixed rate Z=0.50"),
    SettlementRule("SCS",  1250, 0.45, 0.40, 0.50, 0.45, enabled=True, description="SCS — threshold $1,250"),
    SettlementRule("RSG",  0,    0.60, 0.55, 0.60, 0.55, mark_am=True, enabled=True, description="RSG — mark AM, Z=0.60"),
    SettlementRule("NDH",  0,    0,    0,    0,    0,    copy_z_to_ak=True, enabled=True, description="NDH — copy Z to AK"),
    SettlementRule("GSG",  0,    0,    0,    0,    0,    copy_z_to_ak=True, enabled=True, description="GSG — copy Z to AK"),
    SettlementRule("CAV",  0,    0,    0,    0,    0,    review_column=19, mark_am=True, value_to_review="7GMC01", copy_z_to_ak=True, enabled=True, description="CAV — mark AM if 7GMC01, else copy Z to AK"),
    SettlementRule("CAQ",  0,    0,    0,    0,    0,    copy_z_to_ak=True, enabled=True, description="CAQ — copy Z to AK"),
    SettlementRule("CSF",  0,    0,    0,    0,    0,    copy_z_to_ak=True, enabled=True, description="CSF — copy Z to AK"),
]
